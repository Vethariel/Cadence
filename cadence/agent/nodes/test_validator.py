...
def _make_note(t, pitch=65, section="drop", velocity=90):
    return RhythmEvent(
        t=t, type="note", pitch=pitch,
        duration_ms=200, velocity=velocity,
        beat_index=0, section=section,
    )

def _make_drum(t, section="drop", velocity=100):
    return RhythmEvent(
        t=t, type="drum_hit", pitch=36,
        duration_ms=100, velocity=velocity,
        beat_index=0, section=section,
    )
...
def test_passes():
    ...
    melody_events = (
        [_make_note(i * 400, pitch=65 + (i % 5), section="intro", velocity=72) for i in range(8)] +
        [_make_note(3200 + i * 200, pitch=65 + (i % 5), section="drop", velocity=98) for i in range(60)] +
        [_make_note(15200 + i * 400, pitch=65 + (i % 3), section="outro", velocity=78) for i in range(28)]
    )
    # Drums cubren las tres secciones activas
    drum_events = (
        [_make_drum(i * 200, section="intro", velocity=75) for i in range(15)] +
        [_make_drum(3200 + i * 100, section="drop", velocity=105) for i in range(80)] +
        [_make_drum(15200 + i * 200, section="outro", velocity=80) for i in range(15)]
    )
    tracks = [
...
def test_flat_dynamics_fails():
    """Velocity idéntica en todas las secciones — falla dynamic_range."""
    melody_events = (
        [_make_note(i * 200, pitch=65 + (i % 4), section="intro", velocity=80) for i in range(10)] +
        [_make_note(2000 + i * 200, pitch=65 + (i % 4), section="drop", velocity=80) for i in range(20)] +
        [_make_note(6000 + i * 200, pitch=65 + (i % 4), section="outro", velocity=80) for i in range(10)]
    )
    tracks = [
        Track(id="melody", instrument="Lead", midi_channel=0, role="lead", events=melody_events),
        Track(id="drums", instrument="Drums", midi_channel=9, role="rhythm",
              events=[_make_drum(i * 100, section="drop", velocity=80) for i in range(20)]),
        Track(id="bass", instrument="Bass", midi_channel=1, role="bass",
              events=[_make_note(i * 400, pitch=41, section="drop", velocity=80) for i in range(10)]),
    ]
    result = validator_node(_make_base_state(tracks))
    v = result["validation_result"]
